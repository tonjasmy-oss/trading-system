"""
SUI Supertrend 多周期共振策略（SUI/USDT 专用优化）

基于 ATR 的 Supertrend 超级趋势指标，配合 30m/4h/1d 三周期共振过滤假信号。
...
class MultiTFSupertrend:
    ...
    # 默认三周期配置（基于 SUI/USDT 回测优化，加密市场适用较小 multiplier）
    DEFAULT_TIMEFRAMES = [
        {"name": "30m", "period": 10, "multiplier": 0.8},   # 短周期灵敏
        {"name": "4h",  "period": 10, "multiplier": 1.0},   # 中期平衡
        {"name": "1d",  "period": 20, "multiplier": 1.5},   # 长周期稳健
    ]
...
class SUISupertrendStrategy(Strategy):
    """
    SUI Supertrend 策略（SUI/USDT 专用） — 兼容 strategies.py 的 Strategy 基类。
...
def print_stats(stats: Dict):
    """打印回测统计"""
    print("\n" + "=" * 60)
    print("  SUI Supertrend 多周期共振策略（SUI/USDT 专用优化） — 回测报告")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SUI Supertrend 多周期共振策略（SUI/USDT 专用优化）回测")
    parser.add_argument("--symbol", default="SUI/USDT", help="交易对")
